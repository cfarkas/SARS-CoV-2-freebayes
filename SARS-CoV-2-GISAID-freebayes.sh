...
if [ "$1" == "-h" ]; then
  echo ""
  echo "Usage: ./`basename $0` [GISAID.fasta] [Reference]"
  echo ""
...
[ $# -eq 0 ] && { echo "Usage: ./`basename $0` [GISAID.fasta] [Reference]"; exit 1; }

if [ $# -ne 2 ]; then
  echo 1>&2 "Usage: ./`basename $0` [GISAID.fasta] [Reference]"
  exit 3
fi
...
echo "Aligning fasta files to reference"
echo ""
samtools faidx ${2}
fasta= ls -1 *.fasta
for fasta in *.fasta; do
minimap2 -ax asm5 -t 50 ${2} ${fasta} > ${fasta}.sam
